...
function getMockConfig(mockDir) {
  let mockConfig = {};

  fs.readdirSync(mockDir).forEach(function(item) {
    if (item.endsWith('.js')) {
      const fileConfig = require(path.join(mockDir, item));
      Object.assign(mockConfig, fileConfig);
    }
  })
...
exports.mock = function (devServer, customBefore, mockDir) {
  ...
  let mockConfig = getMockConfig(mockDir);
  ...
  return function(app) {
    ...
    watcher.on('change', () => {
      ...
      fs.readdirSync(mockDir).forEach(function(item) {
        if (item.endsWith('.js')) {
          cleanCache(path.join(mockDir, item));
        }
      })

      const newMockConfig = getMockConfig(mockDir);
